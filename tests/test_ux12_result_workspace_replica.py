...
class UX12ResultWorkspaceReplicaTests(unittest.TestCase):
    def test_one_workspace_composes_preview_compare_and_selection_without_copying_result_state(self):
        modules = [
            ...
            ROOT / "static/js/workbench/canvas/result-tray-runtime.js",
...
const controller=sandbox.window.WorkbenchCanvasResultWorkspace.create({{document:documentRef,items,tray:{{sessionId:'session-1',initialItems:items}},compare:{{candidates:items}},selection:{{records:[{{id:'sel-1',attempt_id:'a',output_name:'out',ordinal:0,selected:true,favorite:false,rating:4,comment:'good',revision:1}}]}}}});
...
console.log(JSON.stringify({{sameItems:controller.snapshot().itemCount===items.length,trayItems:mounted.children.tray.snapshot().items.length,initialView:initial.dataset.view,previewView:preview.dataset.view,previewHasText:previewHtml.includes('caption'),afterNext:afterNext.activeItem.item_id,compareView:compare.dataset.view,compareTabs:compare.children[0].children[2].children.map(button=>button.dataset.view),hasCompare:compare.children[1].children.length>0,selectionChild:mounted.children.selection!==null}}));
"""
