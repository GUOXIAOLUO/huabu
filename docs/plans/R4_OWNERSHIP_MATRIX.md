...
| connection mutation | GraphMutationService/API for supported connected creation; normal connect drop creates the edge through the connect-nodes command, Classic side effects (group membership/output sync) stay page-owned | GraphMutationService/API for supported connected creation; normal port-drop connect creates edge + target inputNodeIds through the connect-nodes command, Smart loop-input policy stays page-owned | GraphMutationService connect-nodes command (atomic, revision CAS) | PARTIAL | Unified | Migrate the remaining shared connectInputNode callers (auto-connect, output flows) after save-path parity. | graph API tests; connect command contract |
