...
| connection mutation | GraphMutationService/API for supported connected creation; normal connect drop creates the edge through the connect-nodes command, Classic side effects (group membership/output sync) are decided by the shared compatibility policy and applied by the page | GraphMutationService/API for supported connected creation; normal port-drop connect creates edge + target inputNodeIds through the connect-nodes command, Smart loop-input rule lives in the shared compatibility policy and the page applies its projection | GraphMutationService connect-nodes command (atomic, revision CAS) plus `legacy-graph-compatibility.js` as the single owner of the Classic/Smart connect side-effect rules; `node-creation-client.js` owns versioned result projection | PARTIAL | Unified | Migrate the remaining shared connectInputNode callers (auto-connect, output flows) after save-path parity. | graph API tests; connect command contract; `legacy-graph-compatibility.js`; `node-creation-client.js`; legacy graph policy contract (R4-25) |
...
| group membership | move-driven transition and selected-group child-edge handoff delegated to `group-membership.js`; page keeps group type/geometry policy and side effects (no versioned path yet) | `GroupMembershipService` via `WorkbenchNodeClient.setGroupMembership` for the single non-image/non-group drag-in member add; image absorption / group-merge / ungroup stay page-side | `GroupMembershipService.set_membership` command (atomic, revision CAS, audit) plus `group-membership.js` as the business-neutral membership query/transition owner | PARTIAL | Unified | Migrate remove/ungroup and image/group-merge membership onto the boundary; migrate the Classic transition onto the versioned path after geometry parity. | group-membership API/repository/service tests; group-membership route+client contract (R4-26); transition and selected-group handoff behavior tests (R4-39 Wave 4 slices 2/4) |
...
| media rendering | adapter projection; mounted-card teardown/rebuild delegated through RenderRuntime | adapter mount/lifecycle | MediaRenderer + RenderRuntime | PARTIAL | Unified | Move remaining media projection and prove real media parity before page deletion. | `media-renderer.js`; `render-runtime.js`; render lifecycle contract |
| media lifecycle | RenderSweep delegates reconciliation and clear to RenderRuntime; media playback transplant remains adapter projection | adapter re-render/transplant | RenderRuntime + playback helpers | PARTIAL | Unified | Move real media transplant/lifecycle parity. | `render-sweep.js`; `render-runtime.js`; `media-playback-state.js` |
...
| node deletion | versioned mutation for standalone blank Image/Prompt/default Loop/empty Output/empty Group; compatibility for configured/content/connected/group-member/other nodes; local node/edge projection uses the shared graph fragment | versioned mutation for standalone blank Smart Image/Prompt/empty Smart Group/default Smart Loop; compatibility for media/group/history/dependent nodes | NodeMutationService/API plus `canvas-graph-fragment.js` for local node/incident-edge projection | PARTIAL | Unified | Migrate remaining deletion contracts only after group/media parity is explicit. | Canvas-node route tests; Classic/Smart versioned-delete contracts; graph-fragment behavior contract |
...
| file-drop creation | top-level dropped media creates through CreationController; target fill/group layout/save remain adapter-owned | top-level force-new dropped media creates through CreationController; target fill/group layout/save remain adapter-owned | shared traversal/payload/upload plus CreationController -> NodeCreationService | PARTIAL | Unified | Migrate target-node fill and grouped-media lifecycle only after media/group parity is explicit. | `media-drop-payload.js`; `interaction-controller.js`; file-drop creation contract |
...
| clipboard paste | single-node connection-free Image/Prompt paste creates through CreationController (source `clipboard`); multi-node fragments/other shapes remain adapter-owned | single-node connection-free Smart Prompt paste creates through CreationController (source `clipboard`); fragments/smart-image/groups remain adapter-owned | graph fragment + clipboard helper + CreationController -> NodeCreationService | PARTIAL | Unified | Migrate fragments only after batch/edge persistence and smart-image scale parity are explicit; Alt-drag duplicate stays synchronous page-owned. | `canvas-graph-fragment.js`; `interaction-controller.js`; clipboard creation contract |
...
Resolved for the drop path (2026-09-07, card R4-24): the connect drop no longer couples the durable
edge with page side effects on one raw save. The generic `connect-nodes` application command owns the
atomic, revision-safe edge mutation (plus Smart target `inputNodeIds` in the same lock); Classic group
membership/output/generator sync and the Smart loop-input flag policy remain page-owned compatibility
and persist through the page save when they actually changed. The deferral below still covers the
remaining shared `connectInputNode` callers (auto-connect on drag, output flows, loop migration).
Assessed and deferred (2026-09-06, continued): polling
