...
| Smart handoff | initiates retained handoff | destination runtime | compatibility module | COMPAT_ONLY | REMOVE | R4-35: handoff helpers retired from `WorkbenchCanvasEntryCompatibility`; `requiresLegacySmartHandoff` / `legacySmartCanvasUrl` removed, `/static/smart-canvas.html` URL string gone from every JS file. `R4-36` retires the `smart-canvas.html` / `smart-canvas.js` product page after capability verification. | status U6/U7 |
