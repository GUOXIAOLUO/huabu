...
| group membership | page-owned `updateGroupMembership` geometry/connection-handoff (no versioned path yet) | `GroupMembershipService` via `WorkbenchNodeClient.setGroupMembership` for the single non-image/non-group drag-in member add; image absorption / group-merge / ungroup stay page-side | `GroupMembershipService.set_membership` command (atomic, revision CAS, audit) plus `group-membership.js` as the business-neutral membership query | PARTIAL | Unified | Migrate remove/ungroup and image/group-merge membership onto the boundary; Classic membership stays page-side until geometry parity. | group-membership API/repository/service tests; group-membership route+client contract (R4-26) |
...
| Smart handoff | initiates retained handoff | destination runtime | compatibility module | COMPAT_ONLY | REMOVE | R4-34: openCanvas / createCanvas no longer consume the handoff (canvas.html opens every record natively); R4-35 retires the `WorkbenchCanvasEntryCompatibility` handoff helpers. | status U6/U7 |
